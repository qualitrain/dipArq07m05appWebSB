...
public class FiltroMonitoreoWeb extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (!bitacora.isDebugEnabled()) {
            bitacora.info("No se esta monitoreando");
    ...
    private void logDetails(HttpServletRequest request, HttpServletResponse response, 
        ...
        bitacora.debug("Cuerpo Respuesta (Bytes): {} bytes", respBody.length);
        // Nota: No logueamos el cuerpo completo si es HTML muy grande para no saturar el log
        if (respBody.length < 2048) {
            bitacora.debug("Contenido Respuesta: {}", new String(respBody, StandardCharsets.UTF_8));
        }
        bitacora.debug("--------------------------------------------------------------------------");
        bitacora.debug("TIEMPOS DE PROCESAMIENTO:");
        bitacora.debug("T1 (Inicio): {} ns", t1);
        bitacora.debug("T2 (Fin Lógica): {} ns", t2);
        bitacora.debug("T3 (Fin Ciclo): {} ns", t3);
        bitacora.debug("a. Tiempo procesar peticion (T2-T1): {} ms", (t2 - t1) / 1_000_000.0);
